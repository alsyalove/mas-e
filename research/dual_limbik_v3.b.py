...
Axis:
  1 → clarity ↔ ambiguity
  2 → motion  ↔ latent       [v3: bukan 'potential' — latent adalah gerak
                               di dimensi yang tidak bisa dibaca sistem,
                               bukan sekadar motion yang menunggu]
  3 → chaos   ↔ order

Perubahan v3:
  - Axis 2: MOTION_POTENTIAL → MOTION_LATENT (semantik berbeda secara fundamental)
  - Post-ascend reorientation: setelah frame baru lahir, sistem mengkalibrasi
    ulang seluruh state-nya terhadap attractor dari "posisi baru" tersebut.
    Tanpa reorientation, sistem membawa koordinat lama ke dalam frame baru —
    seperti menggunakan peta kota lama setelah berpindah benua.
"""
...
class Axis(Enum):
    CLARITY_AMBIGUITY = 0
    MOTION_LATENT     = 1   # v3: bukan 'potential' — latent = gerak di luar
                            # ruang persepsi sistem. Ascend pada axis ini
                            # berarti ekspansi ruang persepsi itu sendiri.
    CHAOS_ORDER       = 2
...
class State:
    ...
    def reorient(self, anchor: np.ndarray = ATTRACTOR) -> "State":
        """
        Post-ascend reorientation.

        Setelah frame baru lahir (ascend berhasil), sistem berada di
        'dimensi' yang berbeda. Koordinat lama tidak lagi merepresentasikan
        posisi yang akurat terhadap attractor.

        Reorientation melakukan dua hal:
          1. Menghitung ulang arah sistem dari posisi barunya ke attractor
          2. Menerapkan koreksi kecil ke arah tersebut — bukan lompatan besar,
             tapi 'kalibrasi kompas' di titik baru.

        Analoginya: setelah naik ke lantai yang lebih tinggi (ascend),
        kamu perlu melihat ke mana pintu keluar berada dari lantai itu —
        bukan mengikuti arah yang valid dari lantai sebelumnya.
        """
        direction  = anchor - self.values
        mag        = np.linalg.norm(direction)
        if mag < 1e-10:
            return State(self.values.copy())
        # Koreksi kecil — reorientasi, bukan lompatan
        reoriented = self.values + (direction / mag) * 0.08
...
class Seal:
    ...
    def cipher(self, reflection: ReflectionResult,
        ...
        if domain == "pattern":
            ...
            # Khusus MOTION_LATENT: oposisi bukan soal kecepatan tapi visibilitas.
            # Cipher tidak melemahkan axis — ia memperluas ruang persepsi
            # sehingga yang 'latent' bisa mulai terbaca.
            if ax == Axis.MOTION_LATENT:
                # Ekspansi persepsi: semua axis dinaikkan secara proporsional
                # karena 'melihat yang latent' berarti seluruh ruang melebar,
    ...
    def ascend(self, state: State, cipher: CipherResult,
        ...
        Post-ascend reorientation [v3]:
        Setelah frame baru lahir, sistem berada di 'dimensi' berbeda.
        Reorientation mengkalibrasi ulang kompas sistem dari posisi barunya.
        Ini adalah step tambahan — hanya aktif ketika cipher.frame_dissolved.

        Returns: (new_state, reoriented: bool)
        """
        reoriented = False
        ...
        if not cipher.frame_dissolved:
            ...
            # Post-ascend reorientation:
            # Frame baru lahir → kompas dikalibrasi ulang dari posisi baru.
            new_state  = new_state.reorient()
            reoriented = True

        mag = np.linalg.norm(new_state.values)
        if mag > SCALE_MAX:
...
class DualLimbikSystem:
    ...
    def report(self) -> None:
        ...
        for i, log in enumerate(self.step_log):
            ...
            l1 = log["limbik1"]
            l2 = log["limbik2"]
            r1 = "↺" if l1.get("reoriented") else " "
            r2 = "↺" if l2.get("reoriented") else " "
            print(f"\nstep {i+1:02d}  {s}")
            c1 = l1['cipher'].hidden_meaning or "—"
            c2 = l2['cipher'].hidden_meaning or "—"
            print(f"  L1 {r1} cipher : {c1}")
            print(f"  L2 {r2} cipher : {c2}")
...
if __name__ == "__main__":
    ...
    print("\n\n[3] paradox pada axis motion-latent [v3]")
    print("    Motion terdeteksi tapi arahnya berosilasi —")
    print("    karena sistem bergerak tapi tidak bisa membaca geraknya sendiri.")
    s3 = DualLimbikSystem()
    s3.run(steps=9, perturbations=[
        np.array([ 0.2,  0.6,  0.1]),   # motion tinggi
        np.array([ 0.2, -0.5,  0.1]),   # tiba-tiba latent — sign flip
        np.array([ 0.2,  0.5,  0.1]),   # motion lagi → paradox trigger
        np.array([ 0.1,  0.0,  0.3]),   # sistem menunggu — ascend begins
        np.array([ 0.1,  0.1,  0.3]),   # setelah reorientation, mulai stabil
